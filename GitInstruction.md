...
Если кнопка не появилась переходим во вкладку Pull requests (слева вверху вторая). Нажимаем на зелёную кнопку **New pull requests**. В заголовке справа выбираем нужную ветку для сравнения. Появляется кнопка  **Create pull request**.
