...
Перейдём на сайт github.com. Ищем репозиторий который мы хотим клонировать. Жмем на зелёную кнопка Code. Под HTTS копируем строку с адресом.Откроем терминал:

git clone https://github.com/Velocatcher/first.git

После этого Git скопирует репозиторий, находящийся на сервисе GitHub, в наш Внутри папки Lesson 3 появилась новая папка с именем version_control_lection_3. Узнаем, почему у неё такое название.
...
Для того чтобы работать с файлами локально необходимо переместиться в директория репозитория 
```
